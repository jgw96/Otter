...
export async function publishPost(post: string, id?: string) {
    const correctURL = id ? `https://mammoth-server.azurewebsites.net/status?status=${post}&id=${id}&code=${token}&server=${server}` : `https://mammoth-server.azurewebsites.net/status?status=${post}&code=${token}&server=${server}`;
